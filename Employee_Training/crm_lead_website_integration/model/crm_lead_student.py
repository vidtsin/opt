...
class merged_crm_leads(models.Model):
    ...
    mode_of_contact=fields.Selection([('asked_question','Request Information'),('requested_catalog','Requested Catalog'),('book_a_tour','Book A Tour'),('apply_now','Apply Now'),('call','Call'),('walk_in','Walk In')],related='crm_lead.mode_of_contact',string='Mode Of Contact')
...
class crm(models.Model):
    ...
    # Program Id 1
            if args.get('program_id_1',False):
                if args['program_id_1']['program_name']:
                    program_obj1 = self.env['program.details'].search([('internal_ref', '=', args['program_id_1']['program_short_name'])])
                    if not program_obj1:
    ...
    # Program Id 2
            if args.get('program_id_2',False):
                if args['program_id_2']['program_name']:
                    program_obj2 = self.env['program.details'].search([('internal_ref', '=', args['program_id_2']['program_short_name'])])
                    if not program_obj2:
    ...
    # Program Id 3
            if args.get('program_id_3',False):
                if args['program_id_3']['program_name']:
                    program_obj3 = self.env['program.details'].search([('internal_ref', '=', args['program_id_3']['program_short_name'])])
                    if not program_obj3:
    ...
    def action_mark_lost_wizard(self):
        view = self.env.ref('crm_lead_website_integration.view_mark_lost')
        return {
            'name': _('Mark Lost'),
    ...
    def action_mark_delayed_wizard(self):
        view = self.env.ref('crm_lead_website_integration.view_mark_delayed')
        return {
            'name': _('Mark Delayed'),
            'type': 'ir.actions.act_window',
            'view_type': 'form',
    ...
    def action_set_lost(self):
        crm_lead_obj=self.env['crm.lead'].search([('id','=',self.ids[0])])
        crm_stage_id=self.env['crm.stage'].search([('name','=','Lost')]).id
        vals={'stage_id':crm_stage_id}
        if self.lost_reason:
            vals.update({'lost_reason':self.lost_reason})
        if self.lost_date:
            vals.update({'lost_date':self.lost_date})
        crm_lead_obj.write(vals)
        return True

    def action_set_delayed(self):
        crm_lead_obj = self.env['crm.lead'].search([('id', '=', self.ids[0])])
        crm_stage_id = self.env['crm.stage'].search([('name', '=', 'Delayed')]).id
        vals = {'stage_id': crm_stage_id}
        if self.reason_of_delay:
            vals.update({'reason_of_delay': self.reason_of_delay})
        if self.delay_date:
            vals.update({'delay_date': self.delay_date})
        if self.new_start_date:
            vals.update({'new_start_date': self.new_start_date})
        if self.expected_return_date:
            vals.update({'expected_return_date': self.expected_return_date})
        crm_lead_obj.write(vals)
        return True
...
class MergeOpportunity(models.TransientModel):
    ...
    def action_merge(self):
        self.ensure_one()
        merge_opportunity = self.opportunity_ids.with_context(con_opp_merge=True).merge_opportunity(self.user_id.id, self.team_id.id)
        for rec in self._context.get('active_ids', []):
            self.env['merged.crm.leads'].with_context(con_opp_merge=True).create(
                {'crm_lead_id': merge_opportunity.id, 'crm_lead': rec})
...
class Lead2OpportunityPartner(models.TransientModel):
    ...
    def action_apply(self):
        ...
        if self.name == 'merge':
            ...
            for rec in self._context.get('active_ids', []):
                print rec
                self.env['merged.crm.leads'].with_context(con_opp_merge= True).create({'crm_lead_id':leads.id,'crm_lead':rec})
            if leads.type == "lead":
