...
class sale_workflow_process(models.Model):
    _name = "shopify.sale.workflow.process.ept"
    ...
    @api.model
    def auto_workflow_process(self,shopify_auto_workflow_id=False,ids=[]):            
        with Environment.manage():
            ...
            workflow_process_obj=env_thread1['shopify.sale.workflow.process.ept']
            if not shopify_auto_workflow_id:
                work_flow_process_records=workflow_process_obj.search([])
            else:
                work_flow_process_records=workflow_process_obj.browse(shopify_auto_workflow_id)
            ...
            for work_flow_process_record in work_flow_process_records:
                if not ids:
                    orders=sale_order_obj.search([('shopify_auto_workflow_id','=',work_flow_process_record.id),('state','not in',('done','cancel','shipping_except','invoice_except')),('invoice_status','!=','invoiced')])#('invoiced','=',False)
                else:
                    orders=sale_order_obj.search([('shopify_auto_workflow_id','=',work_flow_process_record.id),('id','in',ids)]) 
                if not orders:
                    continue
                for order in orders:
                    ...
                    if not sale_order_line_obj.search([('product_id.invoice_policy','!=','delivery'),('order_id','in',order.ids)]):
                        continue    
