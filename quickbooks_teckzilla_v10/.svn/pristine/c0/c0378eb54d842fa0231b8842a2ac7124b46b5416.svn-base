...
class customer_sign_up(http.Controller):
    ...
    def quick_book_page(self, **kwargs):
        ...
        return request.render("quickbooks_teckzilla_v10.quick_book_page", values)
    ...
    def add_quick_b_data(self, **post):
        ...
        if authorize_url:
            ...
            return request.render("quickbooks_teckzilla_v10.quick_book_page")
    ...
    def customer_invoice(self,invoices):
        ...
        for invoice in invoices:
            ...
            customer_val = {
                'name': name,
                'customer_quickbook_id': customer_quickbook_id,
                'property_account_receivable_id': 73,
                'property_account_payable_id': 72
            }
    ...
    def add_export_cust_data(self, **post):
        ...
        session_manager = Oauth2SessionManager(
            ...
            client_secret=config_ids.clientsecret,
            base_url='https://localhost:8069/page/quick_book',
        )
        print ("session_manager...........", session_manager)
        callback_url = 'https://localhost:8069/page/quick_book'
        authorize_url = session_manager.get_authorize_url(callback_url)
    ...
    def add_quick_b_vendor(self, **post):
        ...
        if authorize_url:
            ...
            return request.website.render("quickbooks_teckzilla_v10.quick_book_page")
    ...
    def add_quick_b_account(self, **post):
        ...
        if authorize_url:
            ...
            return request.website.render("quickbooks_teckzilla_v10.quick_book_page",values)
...
#        tags = tag.getElementsByTagName('Customer')
    ...
    def export_customer(self):
        ...
        if request.env.context.get('customer_ids',False):
            ...
            update = True
            customer_ids = partner_obj.search([('quick_id','=',False),('quick_export','=',False),('faulty','=',False)])
        ...
        for customer_data in customer_ids:
            ...
            cust_data += """<Customer xmlns="http://schema.intuit.com/finance/v3" domain="QBO" sparse="false">
                            ...
                            </Customer>"""% (customer_data.name,customer_data.parent_id.name or '',customer_data.name,customer_data.phone,customer_data.email,customer_data.street,customer_data.street2 or '',customer_data.city,customer_data.country_id.name,customer_data.zip)
            ...
            r = oauth.post(getresource,data = cust_data,headers=headers)
            ...
            if r.status_code==200:
                ...
                if quick_id:
                    cust_id = customer_data.write({'faulty':False,'faulty_reason':False,'quick_export' : True,'quick_id': quick_id})
...
#            update_customer_ids =[10]
            for update_customer_data in update_customer_ids:
                ...
                up_cust_data += """<Customer xmlns="http://schema.intuit.com/finance/v3" domain="QBO" sparse="false">
                                ...
                                </Customer>"""% (update_customer_data.quick_id,update_customer_data.name,update_customer_data.parent_id.name or '',update_customer_data.name,phone,email,update_customer_data.street,update_customer_data.street2 or '',update_customer_data.city,update_customer_data.country_id.name,update_customer_data.zip)
...
#                    if product_data.categ_id.property_stock_account_input_categ:
                ...
                IncomeAccountRef = acc_ids.quick_acc_id
                if not IncomeAccountRef:
                    IncomeAccountRef = ''
                ExpenseAccountRef = acc_exp_ids.quick_acc_id
                if not ExpenseAccountRef:
                    ExpenseAccountRef = ''
                AssetAccountRef = acc_asset_ids.quick_acc_id
...
#                        </Item>"""% (product_data.name,product_data.lst_price,product_data.qty_available,cr_date)
                ...
                prod_data += """<Item xmlns="http://schema.intuit.com/finance/v3" sparse="false">
                            ...
                            <PurchaseCost>%s</PurchaseCost>
                            <ExpenseAccountRef>%s</ExpenseAccountRef>
                            <AssetAccountRef>%s</AssetAccountRef>
                            <Type>Inventory</Type>
                            <TrackQtyOnHand>true</TrackQtyOnHand>,
                            <QtyOnHand>%s</QtyOnHand>,
                            <InvStartDate>%s</InvStartDate>
                        </Item>"""% (product_data.name,IncomeAccountRef,product_data.lst_price,ExpenseAccountRef,AssetAccountRef,product_data.qty_available,cr_date)
            ...
            if product_data.type == 'service':
                ...
                prod_data += """<Item xmlns="http://schema.intuit.com/finance/v3" sparse="false">
                    ...
                    </Item>"""% (product_data.name,acc_ids.quick_acc_id,product_data.lst_price)
