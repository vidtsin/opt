...
class DonationDonation(models.Model):
    ...
    # compute function will calculate the total in donation of donor.
    @api.multi
    @api.depends(
    ...
    # function default_get will search the condition in account.journal and update in donation.donation
    @api.model
    def default_get(self, default_fields):
        res = super(DonationDonation,self).default_get (default_fields)
        acc_journal_id=self.env['account.journal'].search([('name','=','Bank'),('type','=','bank')])
        if acc_journal_id:
            res.update({'journal_id':acc_journal_id.id})
        return res

    # create each tax receipts
    @api.multi
    def _prepare_each_tax_receipt(self):
        ...
        return vals
    # create  account move in recurring donation(_other information_)
    @api.multi
    ...
    # function will work on validate button will check the condition based on that will raise and create donation goes to done state.
    @api.multi
    def validate(self):
    ...
    # function will create each tax receipt.
    @api.multi
    ...
    # function will work on save default values button it will pass the value in res.user
    @api.multi
    ...
    # function will work on cancel recurring button if donor want to cancel the future donation.
    @api.multi
    def cancel_recurring_donation(self):
        rec = self.search(
            [('partner_id', '=', self.partner_id.id), ('recurring_template', '=', False),
             ('state', '=', 'draft')])
        rec.write({'state': 'cr'})
        self.cancel_recurring = True
        # self.state = 'cr'
    ...
    # check the condition and display donor name
    @api.multi
    ...
    # for tax receipt option to change.
...
class DonationTaxReceipt(models.Model):
    ...
    # for update the tax receipt for annual of donation.
    @api.model
