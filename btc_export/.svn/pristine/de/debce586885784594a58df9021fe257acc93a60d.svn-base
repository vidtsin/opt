...
class wizard_prospect(models.TransientModel):
    _name = "wizard.prospect.new"

    import_csv = fields.Binary('Import Data')

    @api.multi
    def action_prospect2(self):
        ...
        for row in list:
            ...
            if rownum == 0:
                ...
                if row[2] == '' or len(res_id) == 0:
                    continue
                self._cr.execute("""update res_partner set mobile=%s where id=%s""", (row[2] or '', res_id[0].id))
                self._cr.commit()
