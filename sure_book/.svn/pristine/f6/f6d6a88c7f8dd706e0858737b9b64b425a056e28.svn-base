...
class SaleOrder(models.Model):
    ...
    def _website_product_id_change(self, order_id, product_id, qty=0):
        ...
        data_id = self.env['data.record'].sudo().search([])
        data_list = []
        if data_id:
            for data in data_id:
                if data.res_user_id.id == self._uid:
                    data_list.append(data)
        a = str(data_list[0].start_date)
        b = str(data_list[0].end_date)
        date_format = "%Y-%m-%d"
...
#            'product_uom_qty': qty,
            ...
            'res_partner_id': order.partner_id.id,
            'order_id': order_id,
    ...
    def sale_get_order(self, force_create=False, code=None, update_pricelist=False, force_pricelist=False):
        ...
        if not sale_order and (force_create or code):
            ...
            sale_order = self.env['sale.order'].sudo().create({
                ...
                'property_id': data_list[0].property_id.id,
