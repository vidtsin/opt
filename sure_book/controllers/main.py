...
class TableCompute(object):
    ...
    def property_process(self, properties, ppg=PPG):
        # Compute products positions on the grid
        minpos = 0
        index = 0
        maxy = 0
...
class DashboardWidget(http.Controller):
    
    @http.route('/dashboard_check_in', auth='public', type='http', methods=['POST'], csrf=False)
    def submit(self, **kw):
        print"------------self----------",self
        print"------------post----------",kw['id']
        sale_id = request.env['sale.order'].search([('id','=', kw['id'])])
    ...
    @http.route('/dashboard_check_out', auth='public', type='http', methods=['POST'], csrf=False)
    def submit_out(self, **kw):
        print"------------self----------",self
        print"------------post----------",kw['id']
        sale_id = request.env['sale.order'].search([('id','=', kw['id'])])
        sale_id.merge_invoices()
        if sale_id.state == 'check_out':
            values = "True"
        else:
            values = "False"
        return values
    ...
    @http.route('/dashboard_split_bill', auth='public', type='http', methods=['POST'], csrf=False)
    def submit_split(self, **kw):
        uuu
        print"------------self----------",self
        print"------------post----------",kw['id']
        sale_id = request.env['sale.order'].search([('id','=', kw['id'])])
        sale_id.split_invoices()
        if sale_id.state == 'check_out':
            values = "True"
        else:
            values = "False"
...
class Datarecord(http.Controller):
    ...
    def submit(self, **post):
        ...
        keep = QueryURL('/date_records')
        date_format = "%m/%d/%Y"
        start_date = post['datepicker']
        start_date = datetime.strptime(str(start_date), date_format).strftime('%Y-%m-%d')
        
        end_date = post['datepicker1']
        end_date = datetime.strptime(str(end_date), date_format).strftime('%Y-%m-%d')
        values = {
            'start_date': start_date,
            'end_date': end_date,
            'res_user_id': request.uid,
            
        }
        data_id = request.env['data.record'].search([])
        if data_id:
            for data in data_id:
                if data.res_user_id.id == request.uid:
                    data.unlink()
        data_id = request.env['data.record'].create(values)
        return werkzeug.utils.redirect("/properties")
...
class WebsiteProperty(http.Controller):
    ...
    def properties(self, page=0, search='', ppg=False, **post):
        print"-------post------------",post
        print"-------post------------",request.uid
        if ppg:
            try:
                ppg = int(ppg)
            except ValueError:
        ...
        property_obj = request.env['myallocator.property']
        ...
        domain = Property_obj.search([])
        property_count = len(domain)
        pager = request.website.pager(url=url, total=property_count, page=page, step=ppg, scope=7, url_args=post)
        
        
        
        user_id = user_obj.search([('id','=', request.uid)])
        property_list = []
        if user_id:
            if user_id.property_ids:
                for user in user_id.property_ids:
                    if user.my_property_id.is_property_created:
                        property_list.append(user.my_property_id)
            else:
                properties = Property_obj.search([('is_property_created','=', True)])

        values = {
            'properties': property_list or properties,
            'keep': keep,
            'pager': pager,
            'search_count': property_count, 
            'bins': TableCompute().property_process(property_list or properties, ppg),
            'rows': PPR,
        }
        
        return request.render("sure_book.properties", values)
    
    @http.route(['/properties/change_size'], type='json', auth="public")
    def change_size(self, id, x, y):
        property = request.env['myallocator.property'].browse(id)
        return property.write({'website_size_x': x, 'website_size_y': y})
...
class WebsiteSale(http.Controller):
    ...
    def get_attribute_value_ids(self, product):
        ...
        for variant in product.product_variant_ids:
            ...
            prod_id = request.env['sale.order.line'].search([('is_room','=', True), ('product_id','=', variant.id),('order_id','!=', ''),('state','!=', 'cancel'),('property_id','=', data_list[0].property_id.id)])
            print"---------prod_id----------",prod_id
            if prod_id:
                test_list = []
                for test in prod_id:
                    if test.start_date <= data_list[0].start_date < test.end_date or test.start_date <= data_list[0].end_date < test.end_date:
                        print"---------test_list----------"
                ...
                for test in sorted(test_list):
                    act_morn = request.env['sale.order.line'].browse(test)
                    line_id_new.append(act_morn)
                print"------------line_id_new------------",line_id_new
                print"------------line_id_new------------",len(line_id_new)
                count = 0
                for chk in line_id_new:
                    room = 2
                    count += 1
    ...
    @http.route([
        '/shop',
        '/property/<model("myallocator.property"):property>',
        '/shop/page/<int:page>',
        '/shop/category/<model("product.public.category"):category>',
        '/shop/category/<model("product.public.category"):category>/page/<int:page>'
    ], type='http', auth="public", website=True)
    def shop(self, page=0, category=None, property=None, search='', ppg=False,  **post):
        if ppg:
            try:
                ppg = int(ppg)
            except ValueError:
        ...
        if property:    
            ...
            if data_id:
                for data in data_id:
                    if data.res_user_id.id == request.uid:
                        data.write({'property_id' : property.id})
                        print"---------data----------",data
                        data_list.append(data)
        list = []
        for li in prod_list:
            list.append(li.product_variant_ids)
        print"------------list----------------",list
        p_list = []
        for p in list:
            for q in p:
        ...
        for prod in p_list:
            print"------------prod----------------",prod
            prod_id = request.env['sale.order.line'].search([('is_room','=', True), ('product_id','=', prod.id),('order_id','!=', ''),('state','!=', 'cancel'),('property_id','=', property.id)])
            print"---------prod_id----------",prod_id
            if prod_id:
                test_list = []
                for test in prod_id:
                    ...
                    if test.start_date <= data_list[0].start_date < test.end_date or test.start_date < data_list[0].end_date <= test.end_date:
                        ...
                        display_prod.append(prod.product_tmpl_id.id)
                        print"---------display_prod----------",display_prod
                if not test_list:
                    display_prod.append(prod.product_tmpl_id.id)
                    print"---------display_prod----------",display_prod
                ...
                count = 0
                for chk in line_id_new:
#                    if chk.product_id.room_variant_ids:
#                        print"------chk.product_uom_qty---------",chk.product_uom_qty
#                        for room in chk.product_id.room_variant_ids:
#                            if room.property_id == chk.order_id.property_id:
                        count += 1
                        print"-------count---------",count
                        room = 2
                        print"-------room---------",room
                if count != 0:
                    if (count < room):
                        print"-------prod---------"
                        display_prod.append(prod.product_tmpl_id.id)
                        print"-------display_prod----1-----",display_prod
            else:
                display_prod.append(prod.product_tmpl_id.id)
                print"-------display_prod---2------",display_prod
                
        print"-------display_prod---------",display_prod

        
        unique_display_prod = []    
        for i in display_prod:    
            if not i in unique_display_prod:  
                unique_display_prod.append(i)   
        print unique_display_prod
        uniq = []
        for unique in unique_display_prod:
            unique_temp_id = request.env['product.template'].browse(unique)
            uniq.append(unique_temp_id)
        print"-------uniq---------",uniq    
        ...
        if property != None:
            values = {
                ...
                'bins': TableCompute().process(uniq, ppg),
                'rows': PPR,
                'categories': categs,
